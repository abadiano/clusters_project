...
column_names = [
    ...
    "IsProfitable", "MappedDestination", "SimplifiedDestination", "DestinationRegion"
]

# load the dataset
file_path = r'C:\Users\abadianov\Desktop\Projects\ML\datasetv2_updated_dest.csv'
...
data = data[data['SimplifiedDestination'] != 'Other']
...
data = data.drop(columns=['PolicyNumber', 'TotalPPONetworkFeeCAD',
...
data = data.drop(columns=['ProductGroup', 'ProductID', 'ProductName', 'UnderwriterName', 'CoverageCode',
                          'PolicyDuration', 'TimeToEffective', 'PurchaseMonth', 'EffectiveMonth',
                          'MedicalUnderwritingFlag', 'StandardCustom', 'SingleMulti', 'Category',
                          'InsuredProvince', 'SellerName', 'SalesChannel', 'CoverageName', 'NumberOfInsured', 'ClaimsFrequency', 'MappedDestination', 'Destination'])
...
# Map binary variables to single binary features
# binary_vars = ['StandardCustom', 'SingleMulti', 'Category']
# data['StandardCustom'] = data_original['StandardCustom'].map({'Standard': 0, 'Custom': 1})
# data['SingleMulti'] = data_original['SingleMulti'].map({'Single': 0, 'Multi': 1})
# data['Category'] = data_original['Category'].map({'Medical': 0, 'Combo': 1})
...
data['AgeGroup'] = data_original['AgeGroup'].map(age_group_order)

# Re-identify categorical columns
categorical_cols = data.select_dtypes(include=['object']).columns.tolist()

# Separate low and high cardinality columns before encoding
low_cardinality_cols = [col for col in categorical_cols if data[col].nunique() <= 4]
high_cardinality_cols = [col for col in categorical_cols if data[col].nunique() > 4]
...
excluded_features = ['LossRatio']

# Prepare feature matrix X
X = data.drop(columns=excluded_features, errors='ignore')
...
# Prepare feature matrix X
X = data.drop(columns=excluded_features + ['Cluster_Initial'], errors='ignore')
selected_features = X.columns.tolist()
